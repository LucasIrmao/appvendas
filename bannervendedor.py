...
class BannerVendedor(FloatLayout):
    def __init__(self, **kwargs):
        ...
        link = f'https://appvendas-81a96-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor}"'
        ...
        imagem = ImageButton(source=f"icones/fotos_perfil/{avatar}",
                             ...
                             on_relase=partial(meu_aplicativo.carregar_vendas_vendedor, valor))
        LabelId = LabelButton(text=f"ID Vendedor: {id_vendedor}",
                              pos_hint={"right": 0.9, "top": 0.9}, size_hint=(0.5, 0.5),
                              on_release=partial(meu_aplicativo.carregar_vendas_vendedor, valor))

        LabelVendas = LabelButton(text=f"Total de Vendas: R${total_vendas}",
        ...
        self.add_widget(LabelId)
        self.add_widget(LabelVendas)
